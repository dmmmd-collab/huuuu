...
############## FUNCTION #################
def phuocan():print(f'{trang_nhat}═════════════════════════════════════════════════════')
...
def random_color_code():
    """Trả về 1 màu hex ngẫu nhiên"""
    return f"#{random.randint(0, 0xFFFFFF):06x}"

def HoanThanh(dem, id_job, type_action, msg, xu):
    # Chuyển xu về dạng số, tránh lỗi
    try:
        xu_val = int(xu)
    except (TypeError, ValueError):
        xu_val = 0

    xuNow = '{:,}'.format(xu_val).replace(',', '.')
    timeNow = datetime.now().strftime("%H:%M:%S")

    text = Text()
    # Mỗi phần thêm màu gradient ngẫu nhiên
    text.append(f"[{dem}] ", style=random_color_code())
    text.append(f"{type_action} THÀNH CÔNG | ", style=random_color_code())
    text.append(f"{id_job} | ", style=random_color_code())
    text.append(f"{msg} | ", style=random_color_code())
    text.append(f"{timeNow} | ", style=random_color_code())
    text.append(f"XU: {xuNow}", style=random_color_code())
...
def chongblock(delaybl):
    while delaybl > 0:
        for i in range(6):
            ...
            # Nội dung thông báo
            text.append("KÍCH HOẠT TÍNH NĂNG CHỐNG BLOCK, ĐANG ĐỢI → ", style="bold cyan")
            text.append(str(delaybl), style="bold yellow")
            
            # Dấu chấm động
            dots = ["[.....]", "[•....]", "[••...]", "[•••..]", "[•••••.]", "[••••••]"]
            text.append(f" {dots[i]} ", style="bold magenta")
            
            console.print(text, end='\r', justify="left")
            sleep(0.1)
...
def nghingoi(delaymin, delaymax):
    ...
    while delay > 0:
        for i in range(len(dots_cycle)):
            ...
            # [số giây]
            text.append(f"[{delay}]", style="bold yellow")
            text.append(" ")

            # hiệu ứng loading
            text.append(dots_cycle[i], style="bold cyan")
...
while True:
    if os.path.exists('infoTDS_Jiray.txt'):
        ...
        if isinstance(log, (list, tuple)):
            ...
            print(f"{do_nhat}Đăng nhập TDS thất bại!")
            break
...
while True:
    if os.path.exists('infoCKIG_Jiray.txt'):
        ...
        # File chưa tồn tại, tạo mới và gọi hàm nhập IG
        open('infoCKIG_Jiray.txt', 'a').close()
        InputInstagram()
        break
        ...
        for cookie in linesCookie:
            ...
            while True:
                ...
                if nv == 1:
                   ...
                   if 'error' in get:
                       try:
                           countdown = get.get('countdown', 0)
                           print(f'{do_nhat}Đang Lấy Nhiệm Vụ FOLLOW, Countdown : {trang_nhat}{countdown}         ', end='\r')
                           if int(countdown) > 7:
                              for i in range(countdown, -1, -1):
                                 print(f'{xanh_la_nhat}Đang chờ {vang}{i} giây...            ', end='\r')
                                 time.sleep(1)
                       except Exception as e:
                           msg = get.get('error', str(e))
                           print(f'{do_nhat}Đang Get Nhiệm Vụ FOLLOW{trang_nhat}{msg}          ', end='\r')
                           time.sleep(1)
                   else:
                      jobs = get.get('data', [])
                      if not jobs:
                         ...
                         print(f'{xanh_la_nhat}Đã Tìm Thấy {vang}{len(jobs)} {xanh_la_nhat}Nhiệm Vụ FOLLOW                  ', end='\r')
                         for z in jobs:
                             if blocked >= nvblock or doiacc >= doinick:
                                 break
                             ...
                             try:
                                 ...
                                 print(f'{do_nhat}FOLLOW Thất Bại : {job} - Lỗi: {e}               ', end='\r')
                                 error += 1
                                 time.sleep(0.5)
                                 continue
                             if not follow:
                                 ...
                                 time.sleep(0.5)
                                 continue
                             time.sleep(0.5)  # Delay nhẹ để server cập nhật
                             nhan = tds.SendFollowCache(job)
                             if isinstance(nhan, bool):
                                 nhan = (nhan, 0)
                             cache = nhan[1] if nhan[0] else 0
                             if nhan[0] and cache > 10:
                                get_coin = tds.GetCoin(job, 'FOLLOW')
                                if get_coin[0]:
                                    xuNow = get_coin[1]
                                    msg = get_coin[2]
